...
def _get_train_data_loader(batch_size, training_dir, is_distributed):
    ...
    for sent in sentences:
        encoded_dict = tokenizer.encode_plus(
            ...
            max_length=MAX_LEN,
            padding=True,  # Manually pad to max length
            return_attention_mask=True,
            return_tensors='pt',
            truncation=True
        )
        
        input_id = encoded_dict['input_ids'].squeeze().tolist()
        attention_mask = encoded_dict['attention_mask'].squeeze().tolist()

        if len(input_id) > MAX_LEN:
            input_id = input_id[:MAX_LEN]
            attention_mask = attention_mask[:MAX_LEN]
        else:
            input_id = input_id + [tokenizer.pad_token_id] * (MAX_LEN - len(input_id))
            attention_mask = attention_mask + [0] * (MAX_LEN - len(attention_mask))

        input_ids.append(torch.tensor(input_id).unsqueeze(0))
...
def train(args):
    ...
    remote_server_uri = "http://ec2-13-60-68-88.eu-north-1.compute.amazonaws.com:5000/"
    ...
    mlflow.set_experiment("Training BERT with Sagemaker")

    with mlflow.start_run():
        mlflow.set_tag("Training Info", "Training BERT on IMDB Dataset")
        mlflow.log_param("model_name", 'bert-base-uncased-sagemaker')
        mlflow.log_param("num_train_epochs", args.epochs)
        mlflow.log_param("train_batch_size", args.batch_size)
        mlflow.log_param("test_batch_size", args.batch_size)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            mlflow.log_metric("average_loss", average_train_loss, step=epoch)
        ...
        mlflow.pytorch.log_model(model, "bert-base-uncased-sagemaker")
...
def f1_score_func(preds, labels):
    ...
    print("preds_flat len",len(preds_flat))
...
def test(model, test_loader, device, epoch):
    ...
    print("true_vals len",len(true_vals))
